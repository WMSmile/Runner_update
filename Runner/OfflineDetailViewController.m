...
- (NSString *)cellDetailTextForItem:(MAOfflineItem *)item
{
    ...
    if (![self.downloadingItems containsObject:item])
    {
        ...
        {
            detailText = [NSString stringWithFormat:@"%lldM/%lldM", item.downloadedSize/(1024*1024), item.size/(1024*1024)];
        }
        else
        {
            detailText = [NSString stringWithFormat:@"大小:%lldM", item.size/(1024*1024)];
        }
    }
    else
    {
...
- (void)initNavigationBar
{
    ...
    [titleButton setTitle:@"离线地图" forState:UIControlStateNormal];
...
- (void)initToolBar
{
    UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT - 44, SCREEN_WIDTH, 44)];
    bgView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:bgView];
    
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(10, SCREEN_HEIGHT - 40, SCREEN_WIDTH/2 - 15, 36)];
    [cancelButton addTarget:self action:@selector(cancelAllAction) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:cancelButton];
    
    UIButton *clearButton = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2 + 5, SCREEN_HEIGHT - 40, SCREEN_WIDTH/2 - 15, 36)];
    ...
    [clearButton.layer setBorderWidth:0.5f];
    [clearButton.layer setBorderColor:[UIColor grayColor].CGColor];
    [clearButton.layer setCornerRadius:5.f];
    
    [cancelButton setTitle:@"取消全部" forState:UIControlStateNormal];
    ...
    [clearButton setTitleColor:[UIColor greenColor] forState:UIControlStateHighlighted];
}
